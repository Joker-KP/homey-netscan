...
module.exports = class TcpIpDevice extends Homey.Device {
    async onInit() {
        this.fixCapabilities();
        this.initVariables();
        const fewSeconds = 2
        const startupDelay = this.withJitter(fewSeconds * 1000, 0.9)
    ...
    withJitter(baseValue, jitterFraction = 0.2) {
        ...
        return new Promise((resolve) => {
            ...
            socket.once('error', (error) => {
                const reason = error?.code
                    ...
                    : `unknown error: ${utils.varToString(this.homey, error)}`;
    ...
    handleAvaiabilityChange(isOnline) {
        ...
        if ((this.wasOnline === null) || this.wasOnline != isOnline) {
            ...
            // trigger the action
            if (isOnline)
                this.driver.deviceCameOnline(this);
            else
                this.driver.deviceWentOffline(this);
        }
        else {
